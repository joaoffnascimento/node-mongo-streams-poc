...
class DocumentGenerator extends Readable {
  ...
  constructor(options: { total?: number } = {}) {
    ...
    this.total =
      options.total || parseInt(process.env.TOTAL_DOCUMENTS || '1000000');
  }
...
class ProgressTransform extends Transform {
  ...
  _transform(document: any, _encoding: BufferEncoding, callback: Function) {
    ...
    if (this.processed % 10000 === 0) {
      ...
      this.logger.info(
        `Seeding progress: ${this.processed.toLocaleString()}/${this.total.toLocaleString()} (${progress}%) | ${rate} docs/sec | ETA: ${eta}s`
      );
    }
...
export class SeedDatabaseUseCase {
  ...
  async execute(
    ...
    const totalDocuments =
      request.totalDocuments ||
      parseInt(process.env.TOTAL_DOCUMENTS || '1000000');
    const batchSize =
      request.batchSize || parseInt(process.env.SEED_BATCH_SIZE || '5000');
    ...
    try {
      ...
      const documentsPerSecond = Math.round(
        totalDocuments / (metrics.totalTime / 1000)
      );
